...
export default function DashboardExactMatch() {
  ...
  const initializeProgress = () => {
    if (user?.id) {
      ...
      if (!userProgress.totalXP) {
        userProgress.totalXP = 0
        userProgress.level = 1
        userProgress.coins = 100
        localStorage.setItem(`progress-${user.id}`, JSON.stringify(userProgress))
      }
    ...
    try {
      if (user?.id) {
        ...
        if (storedChildren.length > 0) {
          ...
          loadChallenges(storedChildren[0].id)
        }
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen relative">
      ...
      <div
        ...
        style={{
          backgroundImage: "url(/images/kids-learning-background.jpg)",
      ...
      <div className="relative z-10 container mx-auto px-4 py-6 max-w-7xl">
        ...
        <header className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button variant="ghost" size="icon" className="rounded-full hover:bg-purple-100" onClick={() => {
              clearUserSession()
              router.push('/')
            }}>
